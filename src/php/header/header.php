    ...
    <header class="py-3 bg-ticketBlue text-white">
        <div class="row mx-3 d-flex justify-content-sm-between align-items-center">
            ...
            <div class="d-none d-md-inline text-md-center col-sm-8">
                <h1><i class="fas fa-ticket-alt"></i> UNIVERSITYTICKET <i class="fas fa-ticket-alt"></i></h1>
                <p class="text-md-center mx-5">Biglietti, Concerti, Spettacolo, Sport & Cultura</p>
            </div>
            <div class="col-6 col-md-2 text-right">
                <?php if (isset($_GET["login"])): ?>
                    <p class="font-weight-bold font-italic text-right text-warning small"><?php echo $_GET["login"] ?></p>
                <?php
                    unset($_GET["login"]);
                else: ?>
                    <a href="login.php" class="btn btn-outline-warning btn-rounded pull-right" role="button">Accedi<i class="fa fa-user ml-3"></i></a>
                <?php endif; ?>
            </div>
        </div>
    </header>
